...
def test_extract_object_details_1():
    ...
    assert schema_object.comment.startswith("Text designations that identify gender.")
...
def test_generate_simulated_data():
    ...
    patient_id = schema_util.generate_simulated_data(
        schema_list[1], "bts:Demographics", "HTA1_0"
    )
    ethnicity = schema_util.generate_simulated_data(
        schema_list[2], "bts:Demographics", "HTA1_0"
    )
    gender = schema_util.generate_simulated_data(
        schema_list[3], "bts:Demographics", "HTA1_0"
    )
    days_to_birth = schema_util.generate_simulated_data(
        schema_list[6], "bts:Demographics", "HTA1_0"
    )
...
def test_get_front_end_schema():
    random.seed(0)
    schema_dict = schema_util.load_htan_schema()
    values = schema_util.get_front_end_simulated_values(
        schema_dict, "bts:Demographics", "HTA1_0"
    )
